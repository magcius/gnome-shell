...
static NPIdentifier extension_has_prefs_id;
...
static gboolean
plugin_extension_has_prefs (PluginObject *obj,
                            NPString      uuid,
                            NPVariant    *result)
{
  gchar *uuid_str;
  ...
  if (!res)
    {
      g_warning ("Failed to retrieve 'prefs': %s", error->message);
      g_error_free (error);
      return FALSE;
    }
                      ...
                      NPVariant       *result)
{
  ...
  else if (name == extension_has_prefs_id)
    {
      if (!NPVARIANT_IS_STRING(args[0])) return FALSE;

      return plugin_extension_has_prefs (obj,
                                         NPVARIANT_TO_STRING(args[0]),
                                         result);
    }
...
init_methods_and_properties (void)
{
  ...
  extension_has_prefs_id = funcs.getstringidentifier ("extensionHasPrefs");
